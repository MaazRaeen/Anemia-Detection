...
# ✅ FIX: handle empty class
    ...
    model = create_model()
    # Note: We create a unique checkpoint file for each fold
    checkpoint = ModelCheckpoint(f'best_densenet_fold_{fold_no}.keras', save_best_only=True, monitor='val_loss', mode='min')
    ...
    model.load_weights(f'best_densenet_fold_{fold_no}.keras')
...
print("\n Final confusion matrix plot saved as 'confusion_matrix_densenet.png'")
